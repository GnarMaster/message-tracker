    ...
    async def 스틸(self, interaction: discord.Interaction, target: discord.Member):
        ...
        try:
            ...
            if job not in ["도적", "암살자", "카피닌자", "의적"]:
                await interaction.edit_original_response(content="❌ 도적 계열만 사용할 수 있는 스킬입니다!")
                return
            ...
            if counter_msg:
                ...
                await interaction.edit_original_response(
                    content=f"🥷 {interaction.user.name}님이 {target.mention} 님을 스틸하려 했으나...\n"
                    f"{counter_msg}"
                )
                return
