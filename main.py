    ...
    try:
        ...
        for idx, row in enumerate(records, start=2): # 헤더가 1행이므로 실제 데이터는 2행부터 시작
            ...
            try:
                ...
                current_gold= safe_int(row.gold("골드",0))
                # 유효한 user_id_from_sheet 일 때만 저장
                if user_id_from_sheet:
                    existing_data[user_id_from_sheet] = (idx, total_messages, current_nickname, mentions, links, images, reels, current_level, current_inlevel_exp, current_gold)
            except Exception as e:
